    ...
    <section className="py-6">
      <div className="text-center mb-10">
        ...
        <div className="inline-flex items-center gap-2 bg-trees-primary/10 px-5 py-2 rounded-full mb-4 border border-trees-primary/20">
          ...
          <span className="text-trees-primary font-medium text-[14px]">
            Tree’s Plywood Professional Testimonials
          </span>
        </div>

        {/* Heading */}
        <h2 className="text-trees-primary text-[24px] sm:text-[30px] font-semibold mb-3">
      ...
      <div className="relative group">
        ...
        <button
          onClick={() => scroll('left')}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white shadow-lg rounded-full p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:scale-110 border border-gray-200"
        ...
        >
          {testimonials.map((testimonial, index) => (
            ...
            >
              ...
              {/* Quote Text */}
              <p className="text-gray-700 text-[14px] leading-relaxed mb-6 min-h-[120px]">
                "{testimonial.quote}"
              </p>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-gray-200">
                ...
                />
                <div>
                  <h4 className="font-semibold text-trees-secondary text-[16px]">{testimonial.name}</h4>
                  <p className="text-gray-600 text-[14px]">{testimonial.role}</p>
                  <p className="text-trees-primary text-[12px] font-medium">{testimonial.city}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <button
          ...
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/90 hover:bg-white shadow-lg rounded-full p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:scale-110 border border-gray-200"
      ...
      <div className="text-center mt-6">
        <p className="text-[12px] text-gray-500 italic">← Scroll to see all {testimonials.length} testimonials →</p>
      </div>
    </section>
  );
}
